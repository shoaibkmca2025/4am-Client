...
const ServicesPage: React.FC = () => {
  ...
  return (
    <div className="pt-24 pb-32 relative overflow-hidden bg-slate-50 dark:bg-brand-dark transition-colors duration-500">
      {/* Decorative Orbs */}
        ...
        <div className="max-w-[1200px] mx-auto">
          ...
          <div className="mb-12 md:mb-16">
            ...
            <motion.h1 
              ...
              className="text-3xl sm:text-4xl md:text-5xl lg:text-[44px] font-display font-bold text-slate-900 dark:text-white tracking-tighter uppercase leading-[1.02] mb-8 heading-interactive"
            >
              THE CORE{' '}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-primary via-indigo-500 to-brand-accent heading-gradient-animated">
                OFFERING.
              </span>
            </motion.h1>
            ...
            <motion.p 
              ...
              className="text-lg md:text-xl text-slate-500 dark:text-slate-400 font-light max-w-2xl leading-relaxed italic"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 mb-24">
            {SERVICES.map((service, index) => (
              <motion.div
                ...
                className="group glass rounded-[1.5rem] p-6 cursor-pointer border border-white/40 dark:border-white/5 hover:shadow-2xl hover:shadow-brand-primary/10 transition-all duration-500 h-full"
              >
                <div className="flex flex-col h-full">
                  <div className="w-10 h-10 bg-white dark:bg-white/5 rounded-2xl flex items-center justify-center text-brand-primary shadow-sm mb-5 group-hover:scale-110 transition-transform duration-500 border border-slate-100 dark:border-white/5">
                    {iconMap[service.icon] || <Zap className="w-6 h-6" />}
                  </div>
                  
                  <h2 className="text-xl md:text-2xl font-display font-bold text-slate-900 dark:text-white uppercase tracking-tight mb-2 group-hover:text-brand-primary transition-colors">
                    {service.title}
                  </h2>
          ...
          <div className="mb-32">
            <div className="text-center mb-10">
              ...
              <h3 className="text-2xl sm:text-3xl md:text-4xl font-display font-bold uppercase tracking-tighter">Choose Your <br /> <span className="text-brand-primary">Growth Tier.</span></h3>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              {pricingTiers.map((tier, i) => (
                <div 
                  key={tier.name}
                  className={`glass rounded-[2rem] p-6 border transition-all duration-500 flex flex-col ${
                    i === 1 
                      ? 'border-brand-primary/60 shadow-premium dark:shadow-premium-dark scale-[1.02] z-10 bg-white/95 dark:bg-brand-dark'
                      : 'border-white/40 dark:border-white/5 shadow-sm bg-white/90 dark:bg-brand-dark/70'
                  }`}
                >
                  <div className="mb-6">
                    <h4 className="text-xl font-display font-bold uppercase mb-2">{tier.name}</h4>
                    <p className="text-sm text-slate-500 dark:text-slate-400 font-medium mb-4">{tier.desc}</p>
                    <div className="text-2xl font-display font-bold text-brand-primary">{tier.price}</div>
                  </div>
                  ...
                  <button 
                    ...
                    className={`w-full py-3 rounded-2xl font-bold uppercase tracking-widest text-[10px] transition-all ${
          ...
          {/* Trust Bar */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 pt-16 border-t border-slate-100 dark:border-white/5">
             {[
            ...
            <div key={i} className="space-y-4">
                 ...
                 <h4 className="text-lg font-display font-bold uppercase tracking-tight">{item.label}</h4>
